...
class Quiz extends React.Component {
  ...
  evaluateProblem = () => {
    ...
    sessionData.setData(this.state.hour + "!" + this.state.minute, this.state.wordProblem, attemptedAnswer, answer)
  ...
  getProblem = () => {
    const newProblemSet = MathHelper.generateTimeProblem(this.props.points);
    //const newProblemSet = MathHelper.generateSubtractionProblem(this.props.points);
    this._isMounted &&
      this.setState({
        problem: newProblemSet.problem,
        hour: newProblemSet.hour,
        minute: newProblemSet.minute,
        problemTemplates: [
          {
            pt: newProblemSet.minute == 0 ? "Set the clock to " + newProblemSet.hour + " O' Clock" : newProblemSet.minute == 30 ? "Set the clock to half past " + newProblemSet.hour : "Set the clock to " + newProblemSet.minute + " past " + newProblemSet.hour,
          },
        ]
      }, () => {
